---
name: our-skill-from-pattern
description: When the same multi-step workflow repeats (user corrections or agent redos), capture it as a new SKILL.md in the user skills directory (~/.claude/skills/ or ~/.cursor/skills/) so future sessions load it automatically.
user-invocable: true
---
...
- A correction sounds like a **policy** (“never use raw SQL here — always the repository layer”) — encode it as an always-on **rule/convention** if it should always apply (see the rules/hooks table below); use a **skill** if it is a procedure with steps.
...
Create the new skill at `~/.claude/skills/<slug>/SKILL.md` (Claude Code) or `~/.cursor/skills/<slug>/SKILL.md` (Cursor). If you keep a central skill repo, add it there (e.g. `skills/<slug>/SKILL.md`) and sync to both tools.
...
- **Description** is specific enough for the agent (Claude Code / Cursor) to **match** the skill when the user describes the task.
...
| **Rule / convention** | Always-on conventions, style, file patterns. (Cursor: `.cursor/rules/`; Claude Code: `CLAUDE.md` / project rules.) |
| **Hook** | Automate after file save / stop events. (Cursor: `.cursor/hooks.json`; Claude Code: `settings.json` hooks.) |
